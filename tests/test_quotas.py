...
def test_quotas_creation(db_connection):
    ...
    for quota in quotas:
        assert quota.purchase_order_id.id in [
            order1.id,
            order2.id,
        ], "The purchase_order_id must be set"

        if quota.purchase_order_id.id == order1.id:
            assert (
                float_compare(quota.amount, order1.quantity_executed - order1.fee_paid)
                == 0
            ), "The quota's amount must be the same of the executed quantity for an order, excluding the fee paid"
            assert (
                float_compare(quota.price, order1.price_avg) == 1
            ), "The quota price must be higher than the average order price"

        if quota.purchase_order_id.id == order2.id:
            assert (
                float_compare(quota.amount, order2.quantity_executed - order2.fee_paid)
                == 0
            ), "The quota's amount must be the same of the executed quantity for an order, excluding the fee paid"
            assert (
                float_compare(quota.price, order2.price_avg) == 1
            ), "The quota price must be higher than the average order price"


def test_get_discounted_price():
    """Testa que o desconto é aplicado corretamente para cotas presas"""
    # Teste 1: Hoje (0 dias) - sem desconto
    hoje = datetime.now()
    assert _get_discounted_price(100000, hoje) == 100000.0

    # Teste 2: 8 dias - 1% desconto
    oito_dias = datetime.now() - timedelta(days=8)
    assert _get_discounted_price(100000, oito_dias) == 99000.0

    # Teste 3: 15 dias - 8% desconto
    quinze_dias = datetime.now() - timedelta(days=15)
    assert _get_discounted_price(100000, quinze_dias) == 92000.0

    # Teste 4: 40 dias - 30% desconto (máximo)
    quarenta_dias = datetime.now() - timedelta(days=40)
    assert _get_discounted_price(100000, quarenta_dias) == 70000.0


def _get_discounted_price(quota_price, created_at):
    """Aplica 1% de desconto por dia após 7 dias presa"""
    days_held = (datetime.now() - created_at).days

    if days_held > 7:
        discount_days = min(days_held - 7, 30)
        return quota_price * (1 - discount_days * 0.01)
